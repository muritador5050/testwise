...
class AttemptService {
    ...
    static async submitAnswer(attemptId, answerData) {
        ...
        const question = attempt.test.questions.find((q) => q.id === answerData.questionId);
        if (!question) {
    ...
    static async completeAttempt(attemptId) {
        ...
        const score = attempt.answers.reduce((total, answer) => total + answer.pointsEarned, 0);
        ...
        const timeSpentRaw = Math.floor((new Date().getTime() - attempt.startedAt.getTime()) / 1000);
    ...
    static async getUserAttempts(userId, testId) {
    ...
    static async getAttemptAnalytics(testId) {
        const [totalAttempts, completedAttempts, inProgressAttempts, timedOutAttempts, averageScore, averageTimeSpent, passRate,] = await Promise.all([
        ...
        return {
            ...
            passRate: completedAttempts > 0 ? (passRate / completedAttempts) * 100 : 0,
        };
    }
    static async getTestPerformanceByUser(testId) {
    ...
    static async getQuestionPerformance(testId) {
    ...
    static async getUserPerformanceHistory(userId) {
        const attempts = await prisma.attempt.findMany({
    ...
    static async getScoreDistribution(testId) {
        const attempts = await prisma.attempt.findMany({
            where: { testId, status: 'COMPLETED' },
        ...
        const distribution = ranges.map((range) => ({
            range: range.label,
            count: attempts.filter((a) => (a.percentScore || 0) >= range.min &&
                (a.percentScore || 0) <= range.max).length,
        }));
        return distribution;
    }
}
